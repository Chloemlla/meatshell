...
fn visit_ingest_slices(
    ...
    while offset < bytes.len() {
        let room = INGEST_FRAME_BUDGET - *ingested_since_checkpoint;
        let end = (offset + room).min(bytes.len());
        *ingested_since_checkpoint += end - offset;
        let checkpoint = *ingested_since_checkpoint == INGEST_FRAME_BUDGET;
        visit(&bytes[offset..end], checkpoint);
        if checkpoint {
...
mod ingest_frame_tests {
    use super::{visit_ingest_slices, INGEST_FRAME_BUDGET};

    fn split_and_count_requests(input: &[u8]) -> (Vec<u8>, usize) {
        let mut since_checkpoint = 0usize;
        let mut joined = Vec::new();
        let mut requests = 0usize;
        let mut dirty_since_request = false;
        visit_ingest_slices(input, &mut since_checkpoint, |slice, checkpoint| {
    ...
    #[test]
    fn exact_frame_budgets_do_not_add_an_empty_tail_request() {
        let input = vec![b'x'; INGEST_FRAME_BUDGET * 2];
        let (joined, requests) = split_and_count_requests(&input);
    ...
    fn a_partial_tail_gets_one_final_request() {
        let input = vec![b'x'; INGEST_FRAME_BUDGET * 2 + 1];
        let (joined, requests) = split_and_count_requests(&input);
        assert_eq!(joined, input);
    ...
    fn checkpoint_budget_carries_across_input_events() {
        ...
        visit_ingest_slices(
            &vec![b'a'; INGEST_FRAME_BUDGET - 1],
            &mut since_checkpoint,
            |_, checkpoint| checkpoints += usize::from(checkpoint),
        );
        visit_ingest_slices(&[b'b'], &mut since_checkpoint, |_, checkpoint| {
            checkpoints += usize::from(checkpoint)
        });
    ...
    // --- Shell event pump (dedicated thread) ---
    {
        ...
        std::thread::spawn(move || {
            ...
            loop {
                ...
                // Split even a single large event at frame checkpoints so the UI
                // can observe intermediate terminal states instead of only its tail.
                let mut remaining_output_bytes: usize = ui_batch
                    ...
                    .sum();
                let has_control_events = ui_batch
                    .iter()
                    .any(|event| !matches!(event, SessionEvent::Output(_)));
                let mut dirty_since_request = false;
                            ...
                            visit_ingest_slices(
                                ...
                                |slice, checkpoint| {
                                    ...
                                    remaining_output_bytes =
                                        remaining_output_bytes.saturating_sub(slice.len());
                                    dirty_since_request = true;
                                    ...
                                    if checkpoint {
                                        ...
                                        // The event channel is intentionally
                                        // unbounded today. Waiting while a large
                                        // backlog exists would only move bytes from
                                        // the terminal buffer into that channel and
                                        // inflate memory, so catch up first and pace
                                        // once the stream's tail is within reach.
