...
fn layout(
    ...
    let max_physical_height = options
        .max_height
        .map(|max_height| max_height * scale_factor);
    ...
    {
        get_elipsis_glyph(font_context).map(|(elipsis_glyph, font_for_elipsis_glyph)| ElisionInfo {
            elipsis_glyph,
    ...
    for para in paragraphs.iter_mut() {
        para.layout.break_all_lines(
            max_physical_width
                .filter(|_| text_wrap != TextWrap::NoWrap)
                .map(|width| width.get()),
        );
        para.layout.align(
    ...
    let height = paragraphs.last().map_or(PhysicalLength::zero(), |p| {
...
impl Layout {
    ...
    fn glyphs_with_elision<'a>(
        ...
        let elipsis_advance = self
            .elision_info
            .as_ref()
            .map(|info| info.elipsis_glyph.advance)
            .unwrap_or(0.0);
        let max_width = self
            .elision_info
            .as_ref()
        ...
        let elipsis = if needs_elision {
            self.elision_info.as_ref().map(|info| {
                ...
                (
                    elipsis_glyph,
                    info.font_for_elipsis_glyph.clone(),
                    font_size,
                )
            })
        } else {
    ...
    fn draw<R: GlyphRenderer>(
        ...
        let scale = item_renderer.scale_factor();
        for paragraph in &self.paragraphs {
            let y = (self.y_offset + paragraph.y).get();
            // Retain a generous overhang margin for accents and stroked text.
            let margin = paragraph.layout.height().max(64. * scale);
            if y - margin > clip.max_y() * scale {
                break;
            }
            if y + paragraph.layout.height() + margin < clip.min_y() * scale {
                continue;
            }
            paragraph.draw(
                self,
...
pub(crate) struct EditorLayoutCache {
    entry: Option<(EditorLayoutKey, Layout)>,
}
...
pub fn draw_text_input(
    ...
    let key = EditorLayoutKey {
        text: text.clone(),
        source: text_input.syntax_source(),
        spans: text_input.syntax_spans(),
        font: text_input.font_request(item_rc),
        scale: scale_factor,
        wrap: text_input.wrap(),
        options: LayoutOptions::new_from_textinput(text_input, Some(width), Some(height)),
        selection: selection_and_color.clone(),
        preedit: !visual_representation.preedit_range.is_empty(),
    };
    let mut cache = text_input.editor_layout_cache.borrow_mut();
    // Only opt-in editor inputs retain layouts; ordinary application fields keep
    // their original lifetime. Scrolling and cursor blinking reuse shaped text.
    let use_cache = !key.source.is_empty();
    if !use_cache || cache.entry.as_ref().is_none_or(|(old, _)| old != &key) {
        let build_layout = || {
            let mut font_ctx = item_renderer.window().context().font_context().borrow_mut();

            // Color-only spans use exactly the same paragraph splitting and font metrics
            // as plain TextInput. Stale results and IME preedit fall back to native text.
            let paragraphs_without_linebreaks = if text_input.syntax_source() == text
                && !text_input.syntax_spans().is_empty()
                && visual_representation.preedit_range.is_empty()
            {
                let spans = parse_syntax_spans(&text, text_input.syntax_spans().as_str());
                let mut offset = 0;
                text.split('\n')
                    .map(|line| {
                        let range = offset..offset + line.len();
                        let first = spans.partition_point(|(span, _)| span.end <= range.start);
                        let last = spans.partition_point(|(span, _)| span.start < range.end);
                        let formatting = spans[first..last]
                            .iter()
                            .filter_map(|(span, color)| {
                                let start = span.start.max(range.start);
                                let end = span.end.min(range.end);
                                (start < end).then(|| i_slint_common::styled_text::FormattedSpan {
                                    range: start - range.start..end - range.start,
                                    style: i_slint_common::styled_text::Style::Color(*color),
                                })
                            })
                            .collect::<Vec<_>>();
                        let selection = selection_and_color.clone().and_then(|(span, color)| {
                            let start = span.start.max(range.start);
                            let end = span.end.min(range.end);
                            (start < end).then(|| (start - range.start..end - range.start, color))
                        });
                        let layout =
                            layout_builder.build(&mut font_ctx, line, selection, formatting, None);
                        offset += line.len() + 1;
                        TextParagraph {
                            range,
                            y: PhysicalLength::default(),
        ...
        cache.entry = Some((key, build_layout()));
    }
    ...
    if render && text_input.line_number_width().get() > 0. {
        ...
        // Only visible line numbers need their own small glyph runs.
        let clip = item_renderer.get_current_clip();
        let top = clip.min_y() * scale_factor.get();
        let bottom = clip.max_y() * scale_factor.get();
        for (index, paragraph) in layout.paragraphs.iter().enumerate() {
            let y = (layout.y_offset + paragraph.y).get();
            if y > bottom {
                break;
            }
            let Some(first) = paragraph.layout.lines().next() else {
                continue;
            };
            if y + first.metrics().line_height < top {
                continue;
            }
            ...
            let paragraphs = create_text_paragraphs(
                &layout_builder,
                &mut font_ctx,
                PlainOrStyledText::Plain(number.into()),
                None,
                Color::default(),
            );
            let mut options = LayoutOptions::new_from_textinput(
                text_input,
                Some(text_input.line_number_width()),
                None,
            );
            options.horizontal_align = TextHorizontalAlignment::Right;
            options.vertical_align = TextVerticalAlignment::Top;
            let mut number_layout = crate::textlayout::sharedparley::layout(
                &layout_builder,
                &mut font_ctx,
                paragraphs,
                scale_factor,
            ...
            drop(font_ctx);
            let number_baseline = number_layout.paragraphs[0]
                .layout
                .lines()
                .next()
                .map_or(0., |line| line.metrics().baseline);
            number_layout.y_offset =
                PhysicalLength::new(y + first.metrics().baseline - number_baseline);
            for p in &number_layout.paragraphs {
                p.draw(
                    &number_layout,
                    item_renderer,
                    &platform_fill_brush,
                    &None,
                    &mut |renderer, font, size, coords, synthesis, brush, y, glyphs| {
                        renderer.draw_glyph_run(font, size, coords, synthesis, brush, y, glyphs);
                    },
                );
            }
        }
...
// Wire format: sorted non-overlapping UTF-8 byte ranges "start:end:ARGB;".
// Validate at the renderer boundary even though the application is trusted.
fn parse_syntax_spans(text: &str, encoded: &str) -> Vec<(Range<usize>, u32)> {
    ...
    for entry in encoded.split(';').filter(|entry| !entry.is_empty()) {
        ...
        let Some((start, end, color)) = parsed else {
            continue;
        };
