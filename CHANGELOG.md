...
- **修复 Windows 使用 `Ctrl+Space` 切换输入法后字母被误识别为 Ctrl 命令（#309）。** 当 Windows 输入法接管快捷键并吞掉 Ctrl 松开事件时，程序现在会直接查询 Windows 的左右 Ctrl 实时按键状态，在下一个普通按键进入 Slint 前补齐 Ctrl release；正常的 `Ctrl+C`、`Ctrl+X` 等终端组合键保持不变。
...
- **Fix letters being treated as Ctrl commands after switching the Windows IME with `Ctrl+Space` (#309).** When the Windows IME consumes the shortcut and hides the Ctrl key-up event, the application now queries Windows' real-time left/right Ctrl key state to restore the missing Ctrl release before the next ordinary key reaches Slint. Real terminal shortcuts such as `Ctrl+C` and `Ctrl+X` remain unchanged.
